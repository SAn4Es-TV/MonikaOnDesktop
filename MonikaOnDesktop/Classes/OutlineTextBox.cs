// Source - https://stackoverflow.com/a/9887123
...
namespace MonikaOnDesktop {
    ...
    new FrameworkPropertyMetadata(-1, FrameworkPropertyMetadataOptions.AffectsRender));
        ...
        private void UpdateFormattedText() {
            ...
            this.formattedText.LineHeight = this.LineHeight;
        ...
        private void EnsureGeometry() {
            if (this.textGeometry != null) return;

            this.EnsureFormattedText();

            // Если мы хотим эффект печатной машинки:
            string displayUri = this.Text;
            if (VisibleCharactersCount >= 0 && VisibleCharactersCount < this.Text.Length) {
                displayUri = this.Text.Substring(0, VisibleCharactersCount);
            }

            // ВАЖНО: Для Run этот подход не сработает напрямую.
            // Для поддержки Run нужно использовать BuildHighlightGeometry, 
            // но это гораздо сложнее в реализации.
